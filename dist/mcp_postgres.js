...
async function createMySqlMcpServer() {
    ...
    server.tool("execute_query", "Execute a Postgres SQL query (read-only) against the database", {
        sql: zod_1.z.string().trim().min(1, "Postgres query is required").describe("The Postgres query to execute"),
    }, async ({ sql }) => {
        const client = await pool.connect();
        try {
